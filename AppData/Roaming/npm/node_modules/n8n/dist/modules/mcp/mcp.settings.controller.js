...
let McpSettingsController = class McpSettingsController {
    ...
    async getMcpEligibleWorkflows(req, res) {
        const supportedTriggerNodeTypes = Object.keys(mcp_constants_1.SUPPORTED_MCP_TRIGGERS);
        const options = {
    ...
    async toggleWorkflowMCPAccess(req, _res, workflowId, dto) {
        ...
        if (dto.availableInMCP) {
            if (!workflow.activeVersionId) {
                throw new bad_request_error_1.BadRequestError('MCP access can only be set for published workflows');
            }
            const nodes = workflow.activeVersion?.nodes ?? [];
            const supportedTrigger = (0, mcp_utils_1.findMcpSupportedTrigger)(nodes);
            if (!supportedTrigger) {
                throw new bad_request_error_1.BadRequestError(`MCP access can only be set for published workflows with one of the following trigger nodes: ${Object.values(mcp_constants_1.SUPPORTED_MCP_TRIGGERS).join(', ')}.`);
            }
        }
