...
from src.pipe_reader import PipeConnection, PipeReader
...
class TaskExecutor:
    ...
    def execute_process(
        ...
        try:
            try:
                ...
                logger.exception("process.start() FAILED")
                raise
            finally:
                write_conn.close()
    ...
    def _all_items(
        ...
        try:
            ...
            TaskExecutor._put_error(
                write_conn, e, stderr_capture.getvalue(), print_args
            )
    ...
    @staticmethod
    def _put_result(write_conn: PipeConnection, result: Items, print_args: PrintArgs):
        ...
        TaskExecutor._send_pipe_message(write_conn, data)
        write_conn.close()
    ...
    @staticmethod
    def _send_pipe_message(write_conn: PipeConnection, data: bytes):
