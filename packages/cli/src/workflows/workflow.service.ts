...
/**
 * Settings keys that are pure metadata and never affect how the ActiveWorkflowManager
 * registers triggers/webhooks for a workflow. Changing one of these must never force a
 * running workflow through a deactivate/reactivate cycle: if that cycle fails for any
 * reason (webhook conflict, external hook error, etc.), `_addToActiveWorkflowManager`
 * rolls the workflow back to inactive - so an unrelated metadata toggle (e.g. "Available
 * in MCP") could silently unpublish an otherwise-healthy workflow.
 * See: workflow settings reactivation should be reserved for settings that actually
 * change trigger/execution behavior.
 */
const SETTINGS_KEYS_EXCLUDED_FROM_REACTIVATION = ['availableInMCP'] as const;
...
export class WorkflowService {
	...
	async update(
		...
		// Check if settings actually changed. Metadata-only keys (e.g. `availableInMCP`) are
		// excluded here on purpose - see SETTINGS_KEYS_EXCLUDED_FROM_REACTIVATION above. They
		// still get persisted normally below, they just must not be able to trigger a
		// reactivation of the live workflow.
		const settingsChanged =
			workflowUpdateData.settings !== undefined &&
			!isEqual(
				omit(workflow.settings, SETTINGS_KEYS_EXCLUDED_FROM_REACTIVATION),
				omit(workflowUpdateData.settings, SETTINGS_KEYS_EXCLUDED_FROM_REACTIVATION),
			);
